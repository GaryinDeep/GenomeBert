    ...
    def load_weights_from_checkpoint(self, checkpoint, mapping=None):
        ...
        for layer, variables in mapping.items():
            ...
            if isinstance(layer, MultiHeadAttention):
                """如果key_size不等于head_size，则可以通过正交矩阵将相应的权重投影到合适的shape。
                """
                count = 2
                if layer.use_bias:
                    count += 2
...
class BERT(Transformer):
    ...
    def apply_embeddings(self, inputs):
        ...
        if self.use_segment_ids:
            ...
            x = self.apply(inputs=[x1, x2, x3], layer=keras.layers.Add, name='Embedding-Token-Segment')    # 进行相应元素的相加
...
#         （但允许自行传入位置id，以实现一些特殊需求）
#         """
#         x_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Token-ALT')            # ref (record_num, seq_size)
#         alt_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Token-Alt-ALT') # alt  (record_num, seq_size)
#         s_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Segment-ALT')     # type (record_num, seq_size)
...
#             embeddings_initializer=self.initializer,
#             mask_zero=True,
#             name='Embedding-Token-ALT'   # 改Embedding-Token
#         )   # ref
#         if self.custom_conv_layer:
#             x1 = self.apply(
#                 inputs=x,
...
#         if self.custom_conv_layer:
#             embedding_inputs = [x1, x2, x3, alt, s] # [seq_len, embedding_size]
#         else:
...
#         x = self.apply(inputs=embedding_inputs, layer=keras.layers.Add, name='Embedding-Token-Segment-ALT') # [seq_len, embedding_size]
#         x = self.apply(
#             inputs=self.simplify([x, p]),
#             layer=PositionEmbedding,
#             input_dim=self.max_position,
...
class Multi_Inputs_Alt_BERT(Transformer):
    ...
    def get_inputs(self):
        ...
        （但允许自行传入位置id，以实现一些特殊需求）
        """
        x_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Token')            # ref (record_num, seq_size)
        alt_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Token-Alt') # alt  (record_num, seq_size)
        s_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Segment')     # type (record_num, seq_size)
    ...
    def apply_embeddings(self, inputs):
        ...
        x = self.apply(
            ...
            embeddings_initializer=self.initializer,
            mask_zero=True,
            name='Embedding-Token'   # 改Embedding-Token
        )   # ref
        if self.custom_conv_layer:
            x1 = self.apply(
                inputs=x,
        ...
        if self.custom_conv_layer:
            s1 = self.apply(
                ...
                strides=1,
                padding='same',
                name='Embedding-Segment-Conv1D-2'
            )
            s2 = self.apply(
                inputs=s,
                layer=keras.layers.Conv1D,
                ...
                # dilation_rate=2,
                padding='same',
                name='Embedding-Segment-Conv1D-3'
            )
            s3 = self.apply(
                inputs=s,
                layer=keras.layers.Conv1D,
                ...
                strides=1,
                padding='same',
                name='Embedding-Segment-Conv1D-5'
            )

        if self.custom_conv_layer:
            # embedding_inputs = [x1, x2, x3, alt1, alt2, alt3, s] # [seq_len, embedding_size]
            # embedding_inputs = [x1, x2, x3, alt, s] # [seq_len, embedding_size] 
            embedding_inputs = [alt1, alt2, alt3, s]
        else:
            embedding_inputs = [x, alt, s]
        ...
        x = self.apply(inputs=embedding_inputs, layer=keras.layers.Add, name='Embedding-Token-Segment') # [seq_len, embedding_size]
        x = self.apply(
            inputs=self.simplify([x, p]),
            layer=PositionEmbedding,
            input_dim=self.max_position,
...
def build_transformer_model(
    ...
    model, application = model.lower(), application.lower()  # 转小写
    ...
    if checkpoint_path is not None:
        transformer.load_weights_from_checkpoint(checkpoint_path) # 加载检查点的权重
