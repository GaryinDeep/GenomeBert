...
def load_tfrecord(record_names, sequence_length=100, num_classes=919, num_parallel_calls=tf.data.experimental.AUTOTUNE):
    ...
    def parse_function(serialized):
        ...
        segment_id = K.zeros_like(masked_sequence, dtype='int64')  # Return a Keras variable with the shape of x filled with zeros. 
        sequence = features['y']
        x = {
...
def load_npz_record(x_data, y_data):
    ...
    def parse_function(x_data, y_data):
        input_token = x_data
        input_segment = K.zeros_like(input_token, dtype='int64') # 全零
        x = {
...
def pred_result_auc_BiPath_list(y_preds, y_label, ngram, pool_size=8):
    all_y_pred = None

    print("y_pred len: ", len(y_preds))
    print("y_label shape: ", y_label.shape) # (455024, 919)
    ...
    y_pred = all_y_pred / len(y_preds) # ngram set 取均值
    y_pred_val = np.reshape(y_pred, (y_pred.shape[0], y_pred.shape[1])) # (455024, 919)
    # y_pred_val = y_pred_val[:, 0:919]
    print("y_pred_val shape: ", y_pred_val.shape)
    np.save("./data/result_{}_gram.npy".format(ngram), {"y_label": y_label, "y_pred": y_pred_val})
    ...
    for result in results:
        ...
        aucs[index] = auc
    print('AUCs')
    print('onco: %.3f' % aucs[0])
    print('TSG: %.3f' % aucs[1])
    print('mean: %.3f' % np.median(aucs))
...
if __name__ == '__main__':
    ...
    _argparser.add_argument(
        ...
        help='The type of the task')
    _argparser.add_argument(
        '--use-position', action='store_true',       # 触发则为True 否则为False
    ...
    train_n = 330471*ngram  # 注意每次 要改
    valid_n = 41309  # 注意每次 要改
    train_n = 343311*ngram  
    valid_n = 29379 
    steps_per_epoch = int(train_n/(GLOBAL_BATCH_SIZE))
    validation_steps = math.ceil(valid_n / (GLOBAL_BATCH_SIZE))
    shuffle_size = steps_per_epoch*GLOBAL_BATCH_SIZE 
    ...
    with strategy.scope():
        ...
        output = Lambda(lambda x: x[:, 0], name='CLS-token')(bert.model.output)  # (H)
        ...
        albert.compile(loss=[tf.keras.losses.BinaryCrossentropy()],   # 取所有类平均
    ...
    if task == 'train':
        ...
        for valid_file in valid_slice_files:
            ...
            x_valid = loaded['x']   # [200000,  1000]
            y_valid = loaded['y']   # [200000,  919)]
            ...
            for ii in range(ngram):
                if only_one_slice is True:
                    ...
                    slice_indexes = []
                    max_slice_seq_len = x_valid.shape[1] // ngram * ngram # 1000
        ...
        pred_result_auc_BiPath_list(y_preds, y_test, ngram, pool_size=pool_size)
