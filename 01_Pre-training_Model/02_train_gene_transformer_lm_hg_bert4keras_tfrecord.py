...
def load_tfrecord(record_names, sequence_length=100, num_parallel_calls=tf.data.experimental.AUTOTUNE, batch_size=32):
    ...
    def parse_function(serialized):
        ...
        y = K.cast(sequence, K.floatx()) # 转换格式
...
if __name__ == '__main__':
    ...
    ngram = _args.ngram
    stride = _args.stride   # 这里的stride与预处理的stride不同，这里的stride应=ngram
    ...
    only_one_slice = True
    vocab_size = len(word_dict_alphabet) + word_index_from + 3
    ...
    config = {
        ...
        "intermediate_size": model_dim * 4,  # feed forward内部维度
    ...
    with strategy.scope():
        albert = build_transformer_model(
            configs=config,
            model='bert',
            with_mlm='linear', # 最后一层激活函数为linear
            application='lm',   # 默认为"encoder",'lm':GPT模式
            return_keras_model=False
        ) 

        albert = albert.model
        albert.summary()
        # SparseCategoricalCrossentropy标签应从0开始的整数  logits:bool=False->是否需要经过激活函数的处理 
    ...
    if is_training is True:
        ...
        dataset = dataset.shuffle(GLOBAL_BATCH_SIZE * shuffle_size, reshuffle_each_iteration=True)
