...
def generate_samples(train_data,
    ...
    if is_sep_mask: # 增加cls与sep
        sequence = np.zeros((batch_len, train_data.shape[1] + 2), dtype=np.int32) # 创建矩阵
        masked_sequence = np.zeros((batch_len, train_data.shape[1] + 2), dtype=np.int32) # 创建矩阵
        ...
        for ii in range(batch_len):  # 打乱
            index = indexes[ii]
            sequence[ii] = train_data[index]
            masked_sequence[ii] = train_data[index]

            choice_index = np.random.choice(len(sequence[ii]), int(len(sequence[ii]) * 0.15)) # Mask 15% tokens
